"""
This script is called by the plugin-wrapper, <%-pluginID%>.js, which passes down the
...
from <%-pluginID%> import <%-pluginID%>

logger = logging.getLogger('<%-pluginID%>')
...
plugin = <%-pluginID%>(webgme, COMMIT_HASH, BRANCH_NAME, ACTIVE_NODE_PATH, ACTIVE_SELECTION_PATHS, NAMESPACE)
